...
def fetch_expanded_sections():
    ...
    geopolitics = call_claude_web_search(
        "현재 시점 기준 글로벌 증시에 영향을 줄 수 있는 주요 지정학 리스크(중동 정세, 미중관계, "
        "기타 주요 분쟁·외교 이슈)를 검색해서 한국어로 3~4문장으로 요약해줘. 사실 위주로, 과장 없이."
    )
    time.sleep(5)
    global_markets = call_claude_web_search(
        "아시아·유럽 주요 증시(닛케이, 상해종합, 항셍, 유로스톡스 등)의 가장 최근 마감된 거래일 "
        "흐름을 검색해서 한국어로 3~4문장으로 요약해줘. 어느 날짜 기준인지 문장 맨 앞에 명시해줘."
    )
    time.sleep(5)
...
def fetch_foreign_flow():
    """가장 최근(직전) 거래일 코스피 외국인/기관/개인 순매매 동향 (매일 실행).
    이 스크립트는 코스피 개장 전(오전 8시)에 실행되므로 '오늘'이 아니라
    이미 마감된 가장 최근 거래일 데이터를 찾아야 한다."""
    return call_claude_web_search(
        "코스피(KOSPI) 시장의 가장 최근 마감된 거래일 기준으로, 외국인·기관·개인 투자자의 "
        "순매수/순매도 규모와 주로 어떤 업종·종목을 사고팔았는지, 그 이유가 무엇인지 검색해서 "
        "한국어로 3~4문장으로 요약해줘. 어느 날짜 기준인지 문장 맨 앞에 명시하고, "
